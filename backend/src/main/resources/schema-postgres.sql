...
CREATE TABLE IF NOT EXISTS staffs (
    ...
    CONSTRAINT ck_staffs_login_id_format
        CHECK (login_id ~ '^[A-Za-z0-9]{1,8}$'),
    CONSTRAINT ck_staffs_login_password_format
        CHECK (login_password ~ '^[A-Za-z0-9]{1,8}$'),
    CONSTRAINT fk_staffs_department
